...
import BQProfit.CustomSimMananger;
...
public class SimulationThread {
	...
	private void loadModels(SimulationManager simulationManager) throws Exception {
		...
		this.selectLeader(simulationManager);
	...
	private void selectLeader(SimulationManager simulationManager) {
		List<DataCenter> dataCenterList = simulationManager.getDataCentersManager().getEdgeDatacenterList();
        boolean isLeaderSelected = false;
		for(DataCenter dc : dataCenterList){
			for(ComputingNode node: dc.nodeList){
				if(node.getServerType().equals("consensus_node")){

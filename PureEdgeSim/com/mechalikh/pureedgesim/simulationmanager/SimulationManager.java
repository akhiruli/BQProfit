...
import BQProfit.CostProfitCalculator;
...
public class SimulationManager extends SimulationManagerAbstract {
	...
	private CostProfitCalculator profitCalculator;
	/**
	...
	public SimulationManager(SimLog simLog, PureEdgeSim pureEdgeSim, int simulationId, int iteration,
		...
		profitCalculator =  new CostProfitCalculator();
	}

	public CostProfitCalculator getProfitCalculator() {
		return profitCalculator;
	}

	public void setProfitCalculator(CostProfitCalculator profitCalculator) {
	...
	public boolean taskFailed(Task task, int phase) {
		...
		if(this.getScenario().getStringOrchAlgorithm().equals("BQPROFIT_ALGO")){
			//For soft deadline tasks we are ok if we can't meet deadline
			if (phase == 3 && task.getActualNetworkTime() + task.getWatingTime() + task.getActualCpuTime() > task
					.getMaxLatency() && task.getDeadlineType().equals("hard")) {
				System.out.println("Latency: network: "+task.getActualNetworkTime()+" Waiting: "+task.getWatingTime()+" CPU: "+ task.getActualCpuTime()+ " deadline: "+task.getMaxLatency());

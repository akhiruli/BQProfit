...
public class CostProfitCalculator {
    ...
    public void updateCpuProfitAuction(Double cpu_usage, CostPlan cost_plan, BidingValue bidingValue){
        cpu_profit += cpu_usage * (bidingValue.getChargePerMi() - cost_plan.getCostPerMi());
        cpu_cost += cpu_usage * cost_plan.getCostPerMi();
        cpu_revenue += cpu_usage *bidingValue.getChargePerMi();
    }

    public void updateIoProfitAuction(Double io_usage, CostPlan cost_plan, BidingValue bidingValue){
        io_profit += io_usage * (bidingValue.getChargePerIO() - cost_plan.getCostPerIo());
        io_cost += io_usage * cost_plan.getCostPerIo();
        io_revenue += io_usage *bidingValue.getChargePerIO();
    }

    public void updateMemoryProfitAuction(Double memor_usage, CostPlan cost_plan, BidingValue bidingValue){
        //Assuming memory is in KB
        memory_profit += (memor_usage/1024)*(bidingValue.getChargePerMemoryMB() - cost_plan.getCostPerMB());
        memory_cost += (memor_usage/1024)*cost_plan.getCostPerMB();
        memory_revenue = (memor_usage/1024)*bidingValue.getChargePerMemoryMB();
    }

    public void updateCpuProfitMKLRR(Double cpu_usage, CostPlan cost_plan, ChargingPlan chargingPlan){
        cpu_profit += cpu_usage * (chargingPlan.getChargePerMi() - cost_plan.getCostPerMi());
        cpu_cost += cpu_usage * cost_plan.getCostPerMi();
        cpu_revenue += cpu_usage *chargingPlan.getChargePerMi();
    }

    public void updateIoProfitMKLRR(Double io_usage, CostPlan cost_plan, ChargingPlan chargingPlan){
        io_profit += io_usage * (chargingPlan.getChargePerIO() - cost_plan.getCostPerIo());
        io_cost += io_usage * cost_plan.getCostPerIo();
        io_revenue += io_usage * chargingPlan.getChargePerIO();
    }

    public void updateMemoryProfitMKLRR(Double memory_usage, CostPlan cost_plan, ChargingPlan chargingPlan){
        //Assuming memory is in KB
        memory_profit += (memory_usage/1024)*(chargingPlan.getChargePerMemoryMB() - cost_plan.getCostPerMB());
        memory_cost += (memory_usage/1024)*cost_plan.getCostPerMB();
        memory_revenue = (memory_usage/1024)*chargingPlan.getChargePerMemoryMB();
    }
}
